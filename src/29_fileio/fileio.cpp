...
int main(int argc, char ** argv)
{
    ...
    while (!quit)
    {
        while(SDL_PollEvent(&e))
        {
            ...
            {
                if (e.key.keysym.sym == SDLK_UP && in_focus > 0)
                {
                    in_focus -= 1;
                } else

                if (e.key.keysym.sym == SDLK_DOWN && in_focus < 9)
                {
                    in_focus += 1;
                } else
        ...
        for (int i=0; i < 10; i++)
        {
            SDL_Color num_c = i == in_focus ? highlight : c;
            
            string s = to_string(userdata[i]);
            number.load_text(w_renderer, font, &s, &num_c);
            number.render(w_renderer, (WW-number.get_width())/2, 30*i + 50);
        }
