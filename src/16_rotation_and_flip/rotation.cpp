...
class Texture
{
    ...
    ~Texture();
    void free();
    void render(SDL_Renderer* renderer, int x, int y, double angle, SDL_Point* center, SDL_RendererFlip flip, SDL_Rect *src);
...
int main(int argc, char **argv)
{
    ...
    load_media(&w_texture, "./arrow.png");
    w_texture.set_blendmode(SDL_BLENDMODE_BLEND);
    bool quit = false;
    SDL_Event e;
    ...
    while (!quit)
    {
        ...
        w_texture.render(w_renderer, 0, 0, angle, NULL, flip, NULL);
        w_texture.set_alpha(alpha);
...
bool init()
{
    ...
    //create window
    window = SDL_CreateWindow("Rotation and Flipping", UDF, UDF, WW, WH, SDL_WINDOW_SHOWN);
...
void Texture::render(SDL_Renderer* renderer, int x, int y, double angle, SDL_Point* center, SDL_RendererFlip flip, SDL_Rect *src)
{
    ...
    SDL_RenderCopyEx(renderer, this->mtexture, src, &dest, angle, center, flip);
}
