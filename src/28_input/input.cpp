...
string prompt_text = "Enter text here";
string user_input = "";
TTF_Font *font;

Text prompt_text_graphic = Text();
Text user_input_graphic = Text();
...
bool load_media()
{
    ...
    prompt_text_graphic.load_text(w_renderer, font, &prompt_text, &c);
    return true;
}
...
int main(int argc, char ** argv)
{
    ...
    while (!quit)
    {
        while(SDL_PollEvent(&e))
        {
            ...
            {
                if (e.key.keysym.sym == SDLK_BACKSPACE && user_input.length() > 0)
                {
                    user_input.pop_back();
                    render_text = true;
                }
        ...
        prompt_text_graphic.render(w_renderer, (WW-prompt_text_graphic.get_width())/2, (WH-prompt_text_graphic.get_height())/2 - 100);

        if (render_text == true)
        {
            if (!user_input.length()) user_input = " ";
            user_input_graphic.load_text(w_renderer, font, &user_input, &c);
            render_text = false;    
        }
        user_input_graphic.render(w_renderer, (WW-user_input_graphic.get_width())/2, (WH-user_input_graphic.get_height())/2);
